...
(function () {
    ...
    function setEnumerationUnits(blockGroups, map, path, colorScale) {
        ...
        function highlight(props) {
            // Change stroke for selected regions
            d3.selectAll(".regions")
                .filter(function (d) {
                    return d.properties.FID_1 === props.FID_1;
                })
                .style("stroke", "#c1121f")
                .style("stroke-width", "2.5");

            // Update the label for the selected region
            setLabel(props);
        }

        function dehighlight(props) {
            // Reset stroke for all regions
            d3.selectAll(".regions")
                .style("stroke", function (d) {
                    return getStyle(d3.select(this), "stroke");
                })
                .style("stroke-width", function (d) {
                    return getStyle(d3.select(this), "stroke-width");
                });
